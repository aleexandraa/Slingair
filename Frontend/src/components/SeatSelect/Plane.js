...
const Plane = ({ }) => {
  ...
  useEffect(() => {
    if (bookingDetails.flightNum !== null) {
      fetch(`/flight/${bookingDetails.flightNum}`)
        .then((res) => res.json())
        .then((json) => {
        ...
        seating.map((seat) => (
          <SeatWrapper key={`seat-${seat.id}`}>
            <label>
              {seat.isAvailable ? (
                <>
                  <Seat type="radio" value={seat.id} name="seat" onChange={(e) => {
                    setClick(seat._id);
                    setBookingDetails({ ...bookingDetails, seat: e.target.value })
                  }} />
                  <Available style={{
                    background:
                      seat._id === click &&
                      "var(--color-alabama-crimson)",
                    color: seat._id === click && "#fff",
                    fontWeight: seat._id === click && 700,
                  }}
                  >{seat.id}</Available>
                </>
              ) : (
